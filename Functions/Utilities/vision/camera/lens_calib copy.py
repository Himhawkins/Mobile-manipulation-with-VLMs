...
def calibrate_camera_interactive():
    ...
    # --- FIXED: Manually prepare object points for the generic calibrateCamera function ---
    # This provides compatibility with older OpenCV versions that lack aruco.calibrateCameraCharuco
    all_object_points = []
    all_board_corners_3d = charuco_board.getChessboardCorners()
    for ids in all_charuco_ids:
        # Get the 3D coordinates of the corners that were detected in this specific view
        obj_points_for_view = all_board_corners_3d[ids.flatten()]
        all_object_points.append(obj_points_for_view)

    # --- UPDATED: Using the core cv2.calibrateCamera for better compatibility ---
    # This function is more fundamental and less likely to be missing in older versions.
